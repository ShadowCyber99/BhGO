...
// @route   GET /api/rides/supermarkets
// @desc    Get list of available supermarkets
router.get('/supermarkets', auth, async (req, res) => {
  try {
    const rRes = await db.query('SELECT * FROM supermarkets');
    res.json(rRes.rows);
  } catch (err) {
    console.error('Supermarkets Error:', err.message);
    res.status(500).json({ error: 'Server error' });
  }
});

// @route   GET /api/rides/supermarkets/:id/items
// @desc    Get grocery items for a specific supermarket
router.get('/supermarkets/:id/items', auth, async (req, res) => {
  try {
    const mRes = await db.query('SELECT * FROM grocery_items WHERE supermarket_id = $1', [req.params.id]);
    res.json(mRes.rows);
  } catch (err) {
    console.error('Grocery Items Error:', err.message);
    res.status(500).json({ error: 'Server error' });
  }
});

// @route   GET /api/rides/pharmacies
// @desc    Get list of available pharmacies
router.get('/pharmacies', auth, async (req, res) => {
  try {
    const rRes = await db.query('SELECT * FROM pharmacies');
    res.json(rRes.rows);
  } catch (err) {
...
// @route   GET /api/rides/pharmacies/:id/items
// @desc    Get medicine items for a specific pharmacy
router.get('/pharmacies/:id/items', auth, async (req, res) => {
  try {
    const mRes = await db.query('SELECT * FROM medicine_items WHERE pharmacy_id = $1', [req.params.id]);
    res.json(mRes.rows);
  } catch (err) {
    console.error('Medicine Items Error:', err.message);
    res.status(500).json({ error: 'Server error' });
  }
});
