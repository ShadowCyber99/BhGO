...
with open('frontend/src/screens/RiderHomeScreen.js', 'r', encoding='utf-8') as f:
    content = f.read()
...
start_marker = "      {activeFlow === 'services' ? (\n        <View style={{ flex: 1, width: '100%' }}>"
end_marker = "      {/* FOOD MARKETPLACE VIEW */}"

start_idx = content.find(start_marker)
end_idx = content.find(end_marker)

if start_idx != -1 and end_idx != -1:
    new_services_ui = """      {activeFlow === 'services' ? (
...
"""
    ...
    new_content = content[:start_idx] + new_services_ui + "\n" + content[end_idx:]
    with open('frontend/src/screens/RiderHomeScreen.js', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully patched services flow!")
else:
    print("Could not find boundaries for patching services flow.")
